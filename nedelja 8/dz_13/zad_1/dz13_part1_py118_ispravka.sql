...
select RouteID, route.FromAirport, route.ToAirport
FROM route 
...
-- 6.   Napisati upit koji prikazuje listu svih letova na kojima lete avioni koji u nazivu imaju oznaku „SW“. 

SELECT * FROM flight
WHERE flight.AircraftID IN (SELECT AircraftID FROM aircraft 
WHERE aircraft.AircraftTypeID IN (SELECT AircraftTypeID FROM aircrafttype WHERE aircrafttype.AircraftName LIKE "%SW%"));

SELECT AircraftTypeID FROM aircrafttype WHERE aircrafttype.AircraftName LIKE "%SW%"
